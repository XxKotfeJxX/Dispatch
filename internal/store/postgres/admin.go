...
func (store *Store) CreateTemplate(ctx context.Context, item *template.Template) error {
	...
	_, err := store.pool.Exec(ctx, `
		INSERT INTO templates(id,name,channel,subject_template,body_template,created_at,updated_at)
		VALUES ($1,$2,$3,NULLIF($4,''),$5,$6,$6)`,
		item.ID, item.Name, item.Channel, item.SubjectTemplate, item.BodyTemplate, now)
	return err
}

func (store *Store) ListTemplates(ctx context.Context) ([]template.Template, error) {
	rows, err := store.pool.Query(ctx, `
		SELECT id,name,channel,COALESCE(subject_template,''),body_template,created_at,updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Name, &item.Channel, &item.SubjectTemplate,
			&item.BodyTemplate, &item.CreatedAt, &item.UpdatedAt); err != nil {
			return nil, err
		}
...
func (store *Store) GetTemplate(ctx context.Context, id string) (template.Template, error) {
	...
	err := store.pool.QueryRow(ctx, `
		...
		&item.ID, &item.Name, &item.Channel, &item.SubjectTemplate,
		&item.BodyTemplate, &item.CreatedAt, &item.UpdatedAt)
	return item, notFound(err)
}

func (store *Store) UpdateTemplate(ctx context.Context, item template.Template) error {
	command, err := store.pool.Exec(ctx, `
		UPDATE templates SET name=$2,channel=$3,subject_template=NULLIF($4,''),
		    body_template=$5,updated_at=now() WHERE id=$1`,
		item.ID, item.Name, item.Channel, item.SubjectTemplate, item.BodyTemplate)
	if err != nil {
		return err
	}
