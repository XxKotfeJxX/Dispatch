...
func (provider MockDecisionProvider) Model() string { return "mock-routing-v1" }
...
func (provider MockDecisionProvider) Decide(_ context.Context, input DecisionInput) (Decision, map[string]any, error) {
	...
	decision := Decision{
		...
		RecommendedChannels: []string{"email"}, Confidence: 0.86,
		ReasonCodes: []string{"mock_classification"},
	}
	switch {
	case strings.Contains(text, "payment") && strings.Contains(text, "fail"):
		...
		decision.RecommendedChannels = []string{"email", "telegram"}
		decision.SendImmediately = true
	...
	case strings.Contains(text, "incident") || strings.Contains(text, "server"):
		...
		decision.RecommendedChannels = []string{"telegram", "webhook"}
		decision.SendImmediately = true
	...
	case strings.Contains(text, "report"):
		...
		decision.RecommendedChannels = []string{"email"}
