...
RUN CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X dispatch/internal/buildinfo.Version=${VERSION}" -o /out/dispatch-api ./cmd/api \
    ...
    && CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X dispatch/internal/buildinfo.Version=${VERSION}" -o /out/dispatch-discord-bridge ./cmd/discord-bridge
...
COPY --from=build /out/dispatch-discord-bridge /usr/local/bin/dispatch-discord-bridge
USER dispatch
EXPOSE 8080
