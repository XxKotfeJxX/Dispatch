...
import { Activity, Bell, LayoutDashboard, PlugZap, Settings, Users } from 'lucide-react'
...
export default function App() {
  ...
  return <div className="min-h-screen bg-[#070b12] text-slate-100">
    <aside className="fixed inset-y-0 left-0 z-20 hidden w-64 border-r border-white/8 bg-[#0a0f18]/95 p-5 md:block">
      <div className="mb-9 flex items-center gap-3"><img src="/dispatch-logo-256.png" alt="Dispatch" className="size-10 rounded-xl"/><div><b className="text-lg">Dispatch</b><p className="text-xs text-slate-500">Operations console</p></div></div>
    ...
    <main className="md:ml-64"><header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-white/8 bg-[#070b12]/85 px-5 backdrop-blur md:px-8"><span className="flex items-center gap-2 text-sm text-slate-500"><img src="/dispatch-logo-256.png" alt="" className="size-8 rounded-lg md:hidden"/>Notification infrastructure</span><span className="badge">single tenant</span></header><div className="mx-auto max-w-7xl p-5 md:p-8"><Routes>
      ...
      <Route path="/recipients" element={<Recipients/>}/><Route path="/rules" element={<Navigate to="/integrations" replace/>}/><Route path="/integrations" element={<Integrations/>}/><Route path="/sources" element={<Navigate to="/integrations" replace/>}/><Route path="/templates" element={<Templates/>}/><Route path="/settings" element={<SettingsPage/>}/>
    </Routes></div></main>
