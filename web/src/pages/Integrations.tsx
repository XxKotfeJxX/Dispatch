...
export default function Integrations(){
  ...
  const [query,setQuery]=useState(''),[selected,setSelected]=useState<Manifest|null>(null)
  ...
  const choose=(manifest:Manifest)=>{
    setNotice('')
    if(manifest.id==='webhook'){setAdvanced(true);return}
    if(manifest.auth==='app_install'){
      ...
      else setNotice(manifest.setup_hint??'Platform application setup is required.')
      return
    }
    ...
    <section className="space-y-4">
      ...
      {categories.map(category=><div className="space-y-3" key={category}><p className="text-xs font-semibold uppercase tracking-[.16em] text-slate-500">{category}</p><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filtered.filter(item=>item.category===category).map(manifest=>{
        ...
        return <article className={`panel flex min-h-72 flex-col ${blocked?'opacity-70':''}`} key={manifest.id}>
          ...
          <div className="mt-auto pt-5"><p className="mb-3 text-xs leading-5 text-slate-500">{manifest.setup_hint}</p><div className="flex gap-2"><button className={blocked?'btn-secondary flex-1':'btn flex-1'} disabled={blocked} onClick={()=>choose(manifest)}>{manifest.auth==='app_install'?<ExternalLink size={15}/>:blocked?<CircleAlert size={15}/>:<Link2 size={15}/>} {manifest.id==='webhook'?'Configure':manifest.auth==='app_install'?'Install':blocked?'Not available':'Connect'}</button>{manifest.documentation&&<a className="btn-secondary px-3" href={manifest.documentation} target="_blank" rel="noreferrer" aria-label={`${manifest.name} documentation`}><ExternalLink size={15}/></a>}</div></div>
        </article>
      })}</div></div>)}
    </section>

    {advanced&&<section className="space-y-3 border-t border-white/8 pt-7"><div className="flex items-center gap-3"><KeyRound className="text-cyan-300" size={20}/><div><h2>Advanced webhook sources</h2><p className="muted">For unsupported services and custom JSON producers.</p></div></div><Sources/></section>}
