...
export default function NotificationDetail(){
 ...
 <div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]"><section className="panel"><h2>Payload</h2><div className="mt-5 rounded-xl bg-black/20 p-4 whitespace-pre-wrap text-sm leading-7 text-slate-300">{x.data.body}</div></section><section className="panel"><h2>Routing</h2><dl className="mt-5 space-y-4 text-sm"><div><dt className="text-slate-500">Event</dt><dd>{x.data.event_type}</dd></div><div><dt className="text-slate-500">Category</dt><dd>{x.data.category||'Pending'}</dd></div><div><dt className="text-slate-500">Requested</dt><dd>{x.data.requested_channels?.join(', ')||'Automatic'}</dd></div></dl></section></div>
