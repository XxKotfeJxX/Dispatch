...
| AI analysis | Prompt injection, sensitive metadata disclosure, runaway input cost, or invalid AI output | Metadata allowlist and value bounds, input/output limits, system instructions separated from untrusted content, provider-side zero-retention flag, fixed taxonomy, structured schema, local validation, confidence gate, deterministic fallback, and no AI control over destinations |
